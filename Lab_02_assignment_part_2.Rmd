...
mlo_simple %>% mutate(annual = rollapply(data = co2, width = 12, FUN = mean,
  ...
  scale_color_manual(values = c("Raw" = "darkblue", "12-month average" = "black"),
                     name = "Smoothing") +
  labs(x = "Year", y = "CO2 concentration (ppm)", 
       title = "Measured and Seasonally Adjusted CO2")
```
...
#   geom_line(...) + # Now add a medium "darkgreen" line for the one-year smoothed data
#   geom_line(...) + # And a thick "darkblue" line for the ten-year smoothed data
